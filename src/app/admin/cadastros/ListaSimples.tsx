...
function LinhaEditavel({ tabela, item }: { tabela: Tabela; item: Lookup }) {
  const atualizarComTabela = atualizarRegistroSimples.bind(null, tabela);
        ...
        <button
          ...
          onClick={() => {
            ...
            startTransition(async () => {
              const resultado = await excluirRegistroSimples(tabela, item.id);
...
export default function ListaSimples({
  ...
  const criarComTabela = criarRegistroSimples.bind(null, tabela);
  const [estado, formAction] = useActionState(criarComTabela, undefined);

  return (
    <section className="rounded-2xl border-2 border-slate-200 bg-white p-5 shadow-sm">
      <h2 className="mb-4 text-lg font-bold text-primary-900">{titulo}</h2>
